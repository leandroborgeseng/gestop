...
async function main() {
  ...
  const permissoes = await Promise.all(
    [
      ...
      ['chamados.gerenciar', 'Gerenciar chamados e ordens de servico', 'ordens-servico'],
  ...
  const naoConformidade = await prisma.naoConformidade.create({
    data: {
      ...
      status: NaoConformidadeStatus.OS_GERADA,
  ...
  const ordemServico = await prisma.ordemServico.create({
    data: {
      ...
      status: OrdemServicoStatus.ATRIBUIDA,
    ...
    data: [
      {
        ...
        motivo: 'OS criada automaticamente a partir de nao conformidade.',
        alteradoPorId: gestor.id,
      },
      {
        entidadeTipo: 'OrdemServico',
        entidadeId: ordemServico.id,
        statusAnterior: OrdemServicoStatus.ABERTA,
        statusNovo: OrdemServicoStatus.ATRIBUIDA,
